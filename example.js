...
var ID_TO_USE = 199;
var TEMPLATE = new Buffer.alloc(498);
var TEMPLATE_PATH = './';
var IMAGE_PATH = './';
...
stdin.on('data', function (key) {
    if (key == '0') {
        ...
        fps.makeTemplate().then(function (template) {
            TEMPLATE = new Buffer.alloc(template);
            console.log('makeTemplate: [' + template.length + '] ' + (new Buffer.alloc(template)).toString('hex'));
        ...
        fps.getImage().then(
            function (image) {
                console.log('getImage: [' + image.length + '] ' + (new Buffer.alloc(image)).toString('hex'));
        ...
        fps.getRawImage().then(
            function (rawImage) {
                console.log('getRawImage: [' + rawImage.length + '] ' + (new Buffer.alloc(rawImage)).toString('hex'));
        ...
        fps.getTemplate(ID_TO_USE).then(
            function (template) {
                TEMPLATE = new Buffer.alloc(template);
                console.log('getTemplate: ID = ' + ID_TO_USE + ' [' + template.length + '] ' + (new Buffer.alloc(template)).toString('hex'));
