...
var GT511C3 = (function () {
    ...
    var mainBuff = new Buffer.alloc(0);

    var parser = function (emitter, buffer) {

        buffer = Buffer.alloc.concat([mainBuff, buffer]);

        if (buffer[0] == RESPONSE_START_CODE_1 && buffer[1] == RESPONSE_START_CODE_2 && buffer.length >= 12) {

            //lets take the 12bytes response
            var response = new Buffer.alloc(buffer.slice(0, 12));

            //remaining bytes
            buffer = new Buffer.alloc(buffer.slice(12));
        ...
        if (buffer[0] == DATA_START_CODE_1 && buffer[1] == DATA_START_CODE_2 && buffer.length >= DATA_PACKET_LENGTH) {
            ...
            data = new Buffer.alloc(data.slice(4, data.length - 2));

            //remaining bytes
            buffer = new Buffer.alloc(buffer.slice(DATA_PACKET_LENGTH));
        ...
        mainBuff = new Buffer.alloc(buffer);
    };
    ...
    GT511C3.prototype.sendData = function (data, len) {
        ...
        return (new Promise(function (resolve, reject) {
            ...
            var buffer = new Buffer.alloc(len + 6);
            ...
            function fun(baudrate, callback) {
                ...
                var open = function () {
                    ...
                    self.serialPort.on('data_packet', function (response) {
                        ...
                        if (response.crc) {
                            response.data = new Buffer.alloc(response.data);
